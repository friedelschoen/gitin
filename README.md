...
| `description`       | A description of the site, typically shown in metadata or as a subheading.                       | `<empty>`                                                                                   |
| `footer`            | Footer text that will appear at the bottom of every generated HTML page.                         | `Generated by <i><code>gitin</code></i>!`                                                   |
| `favicon`           | Path to the favicon file, which will be used in the HTML output.                                 | `favicon.svg`                                                                               |
| `favicontype`       | MIME type of the favicon file, e.g., `image/svg+xml`.                                            | `image/svg+xml`                                                                             |
| `logoicon`          | Path to the logo icon file, which will be displayed in the header of the HTML page.              | `logo.svg`                                                                                  |
| `stylesheet`        | Path to the stylesheet (CSS file) used for styling the generated HTML.                           | `style.css`                                                                                 |
| `highlightcmd`      | Command for highlighting syntax in the HTML output. Uses placeholders for scheme and lexer type. | `chroma --html --html-only --html-lines --html-inline-styles --style=$scheme --lexer=$type` |
| `colorscheme`       | Color scheme to use for syntax highlighting in the HTML output.                                  | `pastie`                                                                                    |
| `pinfiles`          | List of files (by name) that will be pinned in the header of the HTML page.                      | `LICENSE LICENSE.md COPYING README README.md`                                               |
| `limit/commits`     | Maximum number of commits to display in the log view. `-1` indicates no limit.                   | `-1` (no limit)                                                                             |
| `limit/filesize`    | Maximum file size (in bytes) for syntax highlighting. Files larger than this size are skipped.   | `1e+6` (1MB)                                                                                |
| `files/index`       | Name of the root index file, typically the home page of the generated site.                      | `index.html`                                                                                |
| `files/log`         | Name of the file containing the commit and branch/tag log.                                       | `log.html`                                                                                  |
| `files/files`       | Name of the file that will display the file tree.                                                | `tree.html`                                                                                 |
| `files/json`        | Name of the JSON file containing commit data.                                                    | `commits.json`                                                                              |
| `files/commit-atom` | Name of the file used for Atom feeds of commits.                                                 | `atom.xml`                                                                                  |
| `files/tag-atom`    | Name of the file used for Atom feeds of tags or branches.                                        | `tags.xml`                                                                                  |
| `copy/stylesheet`   | Optional path to copy the stylesheet (CSS file).                                                 | `<no action>`                                                                               |
